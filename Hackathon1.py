...
import matplotlib
matplotlib.use('Agg')  # Force Matplotlib à utiliser un backend non-interactif
...
def load_data():
    ...
    try:
        ...
        if "coal_co2" not in hackathon.columns:
            st.error("La colonne 'coal_co2' est manquante dans les données.")
            st.stop()
        return hackathon
...
# Variable qui contient uniquement les valeurs agrégées de WORLD
...
worldstat.rename(
    columns={
        ...
        "other_industry_co2": "Autres industries"
    },
    inplace=True
)

# Table pivot pour avoir les valeurs par année
...
# On change la colonne des années en string sinon il n'est pas content pour la suite
co2consumption["year"] = co2consumption["year"].astype(str)
...
with st.container():
    ...
    col1.write("L'objectif de baisse de CO2 mondial fourni par l'ONU pour l'année 2030 est d'environ **15 milliards de tonnes par an**, comme vous pouvez le constater nous sommes légèrement au-dessus rien de bien inquiétant 😊 (sarcasme).")
    col1.write("\n")
...
with col1:
    fig = px.area(
        data_frame=worldstat,
        x='year',
        y=['Charbon', 'Pétrole', 'Gaz', 'Ciment', 'Torchage', 'Autres industries'],
...
worldstat2000complete["year"] = worldstat2000complete["year"].astype(str)
...
continent2020 = continent2020.loc[
    (continent2020["country"] == "North America") |
    (continent2020["country"] == "Europe") |
    (continent2020["country"] == "Asia") |
    (continent2020["country"] == "Africa") |
    (continent2020["country"] == "Antarctica") |
    (continent2020["country"] == "Oceania") |
    (continent2020["country"] == "South America")
]
...
with col1:
    ...
    sns.barplot(
        ...
        color="plum",
        ax=ax1,
        order=continent2020.sort_values('co2').country
    ).set(title="Répartition des émissions de CO2 des continents en 2020")
    plt.xticks(rotation=45)
    st.pyplot(fig1, use_container_width=True)

### TEXTE DE LA PARTIE
with col2:
    col2.write("Ce que nous constatons dans un premier temps dans le graphique ci-contre, c'est que l'Asie est le continent qui pollue le plus avec plus de 60% des émissions de CO2 mondiales en 2020 (la Chine a elle toute seule représente environ 30% des émissions de CO2 dans le monde).")
    col2.write("\n")
...
resultat = (avions * 0.0136) + (streaming * 0.01) + (alcool * 0.007) + (smartphone * 0.004)
...
st.write("L'impact réel de l'avion a été notre **grande** surprise. Supprimer le transport aérien aurait un impact très faible par rapport à d'autres actions qui pourraient être entreprises. Sachant qu'il s'agit d'un argument souvent utilisé pour culpabiliser les amoureux du voyage, il faut définitivement mettre celui-ci en perspective.")
st.write("\n")
st.write("Pour terminer, comme vous pouvez le constater nous ne proposons pas de solution miracle.\n")
st.write("En effet, les émissions de CO2 sont un problème complexe assujetti à plusieurs variables. Cependant, nous espérons vous avoir diverti avec cette analyse et restons disponibles si vous avez des questions.")
st.write("Pour information, toutes les données sont issues du site : [https://ourworldindata.org/](https://ourworldindata.org/) que nous vous recommandons de visiter !\n")
